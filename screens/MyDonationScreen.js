...
export default class MyDonationScreen extends Component {
   ...
   sendBook=(bookDetails)=>{
     if(bookDetails.request_status === "Exchanged Item"){
       var requestStatus = "Trade Available"
       db.collection("all_donations").doc(bookDetails.doc_id).update({
         "request_status" : "Trade Available"
       })
     ...
     .then((snapshot)=>{
       snapshot.forEach((doc) => {
         ...
         if(requestStatus === "Item Traded"){
           message = this.state.donorName + " sent you the request"
         }else{
       ...
       rightElement={
           ...
           >
             <Text style={{color:'#ffff'}}>{
               item.request_status === "Item Traded" ? "Item Traded" : "Send item"
             }</Text>
           </TouchableOpacity>
         }
       bottomDivider
